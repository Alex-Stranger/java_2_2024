...
public interface AgreementEntityRepository extends JpaRepository<AgreementEntity, Long> {
    ...
    @Query("SELECT a.uuid FROM AgreementEntity a WHERE a.uuid NOT IN " + "(SELECT e.agreementUuid FROM AgreementXmlExportEntity e)")
    List<String> findNotExportedAgreementUuids();

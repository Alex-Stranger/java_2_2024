...
public class RiskTravelCancellationV1Test {
    ...
    void testAgreementCases(String caseFolder) throws Exception {
        String requestPath = "rest/v1/risk_travel_cancellation/" + caseFolder + "/request.json";
        String responsePath = "rest/v1/risk_travel_cancellation/" + caseFolder + "/response.json";
    ...
    static Stream<String> testCaseProvider() {
        try {
            Path basePath = Paths.get(Objects.requireNonNull(
                    AgreementV1Test.class.getClassLoader().getResource("rest/v1/risk_travel_cancellation")
            ).toURI());

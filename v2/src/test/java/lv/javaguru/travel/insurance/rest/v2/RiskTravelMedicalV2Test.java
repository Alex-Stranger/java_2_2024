...
public class RiskTravelMedicalV2Test {
    ...
    void testAgreementCases(String caseFolder) throws Exception {
        String requestPath = "rest/v2/risk_travel_medical/" + caseFolder + "/request.json";
        String responsePath = "rest/v2/risk_travel_medical/" + caseFolder + "/response.json";
    ...
    static Stream<String> testCaseProvider() {
        try {
            Path basePath = Paths.get(Objects.requireNonNull(
                    AgreementV1Test.class.getClassLoader().getResource("rest/v2/risk_travel_medical")
            ).toURI());
            ...
            throw new RuntimeException("Failed to scan risk_travel_medical test folders", e);
        }
    }

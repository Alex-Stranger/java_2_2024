...
public class RisksV2Test {
    ...
    void testAgreementCases(String caseFolder) throws Exception {
        String requestPath = "rest/v2/risks/" + caseFolder + "/request.json";
        String responsePath = "rest/v2/risks/" + caseFolder + "/response.json";
    ...
    static Stream<String> testCaseProvider() {
        try {
            Path basePath = Paths.get(Objects.requireNonNull(
                    AgreementV1Test.class.getClassLoader().getResource("rest/v2/risks")
            ).toURI());
            ...
            throw new RuntimeException("Failed to scan risks test folders", e);
        }
    }
}
